...
import 'package:chest_disease_app/features/login/presentation/view/widgets/rember_me_widget.dart';
...
class _LoginFormWidgetState extends State<LoginFormWidget> {
  @override
  void initState() {
    super.initState(); // ضروري جداً أن تكون في البداية

    // استخدام addPostFrameCallback لضمان أن السياق جاهز وأن الـ Widget مضاف للشجرة
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;

      try {
        // حماية داخلية: لو الـ context اتفصل بسبب الـ didReplace الأبلكيشن مش هيقع
        var cubit = context.read<LoginCubit>();
        await cubit.checkBiometricAvailability();

        if (mounted && cubit.isBiometricAvailable) {
          await cubit.authenticateWithBiometrics();
        }
      } catch (e) {
        // طباعة الخطأ في الـ Console فقط بدون ما يظهر للمستخدم أو يوقع التطبيق
        debugPrint("Biometric background initialization skipped: $e");
      }
    });
  }
